...
const ResetPassword = () => {
    ...
    const BEARER_TOKEN = useContext(userData.token);

    const { register, handleSubmit, formState: { errors } } = useForm();
    const { data, loading, error, status, fetchData } = useAxios('/forgot-password/reset', 'post', {headers:{'Authorization': 'Bearer ' + BEARER_TOKEN}})
                ...
                <div className="bg-white-default rounded-md shadow-1x">
                    <form onSubmit={handleSubmit(onSubmit)}>
                        <div className="py-10 px-[30px] text-black-default">
                            ...
                            <Button classname="w-full mt-8" text="Email Address" />
                        </div>
                    </form>
                </div>
            </div>
